...
%
%  PŘEDPOKLADY:
%    - FSAE_Pushrod_Parametry.xlsx
%    - Vehicle_data_dwpushrod.m
%    - MATLAB R2019b nebo novější
%
%
% Copyright 2026 — FSAE tým / (v2)
% =========================================================================
...
%% ── 0. KONFIGURACE ────────────────────────────────────────────────────────
% Získání cesty ke složce, ve které se právě nachází tento skript
script_dir = fileparts(mfilename('fullpath'));

% Dynamické sestavení cest (bude fungovat na jakémkoliv PC i operačním systému)
EXCEL_FILE  = fullfile(script_dir, 'FSAE_Pushrod_Parametry.xlsx');
OUTPUT_FILE = fullfile(script_dir, 'Scripts_Data', 'Data_Vehicle', 'Vehicle_data_dwpushrod.m');
...
for s = 1:length(sheets)
    ...
    try
        ...
        for row = 1:size(raw, 1)
            ...
            % Přeskočit indexované cesty jako Vehicle.X.Value[1]
            ...
            % Bezpečná kontrola prázdných hodnot
            if fsae_is_empty_value(value_raw)
                continue;
            end

            % Přeskočit Excel formule (=odkaz na jinou buňku)
...
fprintf('\n=================================================================\n');
fprintf(' HOTOVO! Parametry z Excelu jsou přeneseny.\n');
fprintf('=================================================================\n');
